...
string t_java_generator::java_thrift_imports() {
  return
    ...
    "import org.apache.thrift.annotation.*;\n" +
...
void t_java_generator::generate_java_struct_definition(ofstream &out,
  ...
  for (m_iter = members.begin(); m_iter != members.end(); ++m_iter) {
    ...
    indent(out) << "@ThriftField(name=\"" << (*m_iter)->get_name() << "\", type=" << type_to_enum((*m_iter)->get_type()) << ", id=" << (*m_iter)->get_key() << ")" << endl;
    if (bean_style_) {
      indent(out) << "private ";
    } else {
      indent(out) << "public ";
    ...
    if (!bean_style_){
      ...
      for (f_iter = fields.begin(); f_iter != fields.end(); ++f_iter) {
        if ((*f_iter)->get_req() == t_field::T_REQUIRED && !type_can_be_null((*f_iter)->get_type())) {
          out <<
            indent() << "if (" << (*f_iter)->get_name() << " == null) {" << endl <<
